""" Definition of a vocabulary using stemmed words extracted from a
    given string.
"""
...
class Vocabulary(object):
    def __init__(self, text_list, max_size=None):
        ''' Builds a dictionary with the stemmed words extracted from a given
            list of words
        ...
        Returns:
            key_dict: dictionary with stemmed words as keys, and an index as
                      values.
        '''
        ...
        if max_size is not None:
            sorted_hist = sorted(key_hist.items(), key=operator.itemgetter(1),
                                 reverse=True)

            self.key_dict = dict(
                (d[0], i) for d, i in zip(sorted_hist[:max_size],
                                          range(1, max_size+1)))

        self.key_dict['N/A'] = 0

        self.vocabulary_size = len(self.key_dict)
        self.key_list = sorted(self.key_dict.items(),
