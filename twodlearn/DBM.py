...
class RBM(object):
    ...
    def __init__(self, n_inputs, n_units, afunction=None, name=''):
        ...
        self.weights = tf.Variable(
            tf.truncated_normal([n_inputs, n_units], stddev=0.1),
            name='W'+name)
        self.bias_b = tf.Variable(
            tf.truncated_normal([n_units], stddev=0.1),
            name='b'+name)
        self.bias_c = tf.Variable(
            tf.truncated_normal([n_inputs], stddev=0.1),
            name='c'+name)
    ...
    def evaluate_x_given_h(self, input_mat):
        return self.afunction(tf.matmul(input_mat, tf.transpose(self.weights))
                              + self.bias_c)
...
class StackedAutoencoderNet(object):
    def __init__(self, n_inputs, n_outputs, n_hidden, afunction=None, name=''):
        ...
        self.out_layer = LinearLayer(n_hidden[-1], n_outputs,
                                     name='_lin_'+name)
    ...
    def setup(self, batch_size, drop_prob=None, l2_reg_coef=None, inputs=None):
        ...
        drop_prob: placeholder used for specify the probability for dropout.
            If this coefficient is set, then dropout regularization is added
            between all fully connected layers
        ...
        if inputs is None:
            inputs = tf.placeholder(
                tf.float32, shape=(batch_size, self.n_inputs))
        ...
        for l_idx in range(1, len(self.full_layers)):
            ...
            xp_list.append(self.full_layers[l_idx]
                           .evaluate_transpose(h_list[-1]))
        ...
        return AutoencoderNetConf(inputs, labels, y, loss, xp_list, h_list,
                                  xp_losses)
